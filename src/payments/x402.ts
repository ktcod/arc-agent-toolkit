...
/**
 * Build the x402 resource server for Arc, backed by Circle Gateway, and wrap it in the gate.
 *
 * Two Arc-specific pieces, and only two:
 *
 *  - `BatchFacilitatorClient` talks to Circle Gateway's /v1/x402/{verify,settle,supported}
 *    instead of a generic facilitator. Gateway batches EIP-3009 authorizations and settles net
 *    positions onchain, which is what makes sub-cent payments economic.
 *
 *  - `GatewayEvmScheme` extends ExactEvmScheme to merge the facilitator's `extra` into the
 *    payment requirements. The base scheme DISCARDS it, and Gateway buyers need
 *    `extra.verifyingContract` to build the correct EIP-712 signing domain — without it the
 *    buyer signs against the wrong domain and every payment fails verification.
 *
 * The cast on the facilitator is deliberate: @circle-fin/x402-batching re-declares a minimal
 * copy of FacilitatorClient rather than importing @x402/core's, and the two disagree on optional
 * fields (core's ResourceInfo.description is optional, the copy's is required). The runtime
 * shapes are identical; only the declarations differ.
 */
export async function buildResourceServer(config: AppConfig): Promise<x402ResourceServer> {
  ...
  server.register(config.network, new GatewayEvmScheme());
