...
const SERVICE_DESCRIPTION =
  "Pay-per-call data tools for AI agents on Arc, settled in USDC on Arc mainnet via Circle " +
  "Nanopayments. Chain status and canonical Circle contract addresses (free), gas quotes priced " +
  "directly in USDC dollars, deterministic transaction finality using Arc's safe/finalized " +
  "tags, canonical-asset verification against an on-chain registry, and token pool liquidity " +
  "versus claimed market cap.";
 ...
 */
/*
 * Both routes MUST declare no-store. With no cache header at all the edge applies its own policy,
 * which served a stale snapshot from the custom domain for minutes after a deploy while the Worker
 * itself had fresh code — and silently defeats the page's 20s polling, so a "live" view is not.
 */
const NO_STORE = { "cache-control": "no-store" } as const;

/** Monitor needs the resolved Gateway endpoint, not just the payout address. */
function monitorOpts(env: Env): { gatewayUrl: string; network: string; chain: "arc" | "arcTestnet" } {
  const config = loadConfig(env, PRICE_SPECS);
  const supported = SUPPORTED_NETWORKS[config.network];
  return {
    gatewayUrl: config.facilitatorUrl,
    network: config.network,
    chain: supported?.chain ?? "arc",
  };
}
...
app.get("/monitor.json", async (c) => {
  ...
  try {
    const config = loadConfig(env, PRICE_SPECS);
    return c.json(
      await buildSnapshot(config.payTo, PAID_SPECS, monitorOpts(env)),
      200,
      NO_STORE,
    );
  } catch (e) {
    return jsonBody(500, { error: errMessage(e) });
  }
});
...
app.get("/monitor", async (c) => {
  ...
  try {
    const config = loadConfig(env, PRICE_SPECS);
    const snapshot = await buildSnapshot(config.payTo, PAID_SPECS, monitorOpts(env));
    return c.html(renderMonitorHtml(snapshot, tools.length), 200, NO_STORE);
  } catch (e) {
    return jsonBody(500, { error: errMessage(e) });
  }
});
